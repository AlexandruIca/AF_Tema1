...
// starile posibile pentru o celula
...
auto solve() -> int
{
    ...
    while(!g_cells.empty()) {
        ...
        g_cells.pop_front();
        result += current_cell.cost; // aici incrementez suma ceruta

        // pentru toate directiile in care pot sa ma duc
        for(int dir = 0; dir < 4; dir++) {
            int const new_r = current_cell.row + dir_row[dir];
            int const new_c = current_cell.col + dir_col[dir];

            // daca pot sa ajung intr-o celula empty
            if(is_inside(new_r, new_c) && g_matrix[new_r][new_c] == cell_state::empty) {
                g_matrix[new_r][new_c] = cell_state::occupied;
                g_cells.emplace_back(new_r, new_c, current_cell.cost + 1); // costul pana acolo devine +1
            }
        }
    }
