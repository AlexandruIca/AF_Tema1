...
auto read_graph() -> void
{
    ...
    for(int i = 0; i < m; ++i) {
        ...
        g_graph[y].push_back(x); // graf neorientat
    }
}

auto bfs(int const start_node, kind const k) -> void
{
    // calculez distantele pornind de la nodul specificat prin start_node
...
auto main() noexcept -> int
{
    ...
    for(int i = 1; i <= g_num_nodes; ++i) {
        // aceasta este proprietatea care ne spune ca nodul sigur apartine unui drum de cost minim
        if(g_dist_start[i] + g_dist_end[i] == g_dist_start[g_end_node]) {
            // retin nodurile aflate la distanta curenta fata de start_node
            dist_level[g_dist_start[i]].insert(i);
        }
    }

    for(int i = 1; i <= g_dist_start[g_end_node]; ++i) {
        // cand pentru o anumita distanta am doar un nod care se afla pe cel putin un drum de cost minim,
        // atunci sigur se afla pe toate drumurile de cost minim
