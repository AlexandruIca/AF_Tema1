...
matrix_t g_matrix;
matrix_t g_cost; // matricea in care tin costul pentru a ajunge la punctul (i, j)
...
auto read_graph() -> void
{
    ...
    for(int i = 1; i <= n; ++i) {
        for(int j = 1; j <= m; ++j) {
            f >> g_matrix[i][j];
            g_cost[i][j] = g_inf; // prima data peste tot costul e infinit
        }
    }
}
...
// helper care imi acceseaza matricea la punctul p(i, j)
template<typename T>
...
auto solve() -> void
{
    ...
    while(!queue.empty()) {
        ...
        // pentru toate directiile in care ma duc
        for(auto const& dir : directions) {
            ...
            // Daca costul pt noua coordonata pe care ma duc e mai mic decat ce era inainte actualizez costul
            // si inserez noua coordonata la finalul cozii
            ...
            // Daca in directia in care ma duc am copac de acelasi fel si costul e imbunatatit atunci actualizez
            // costul si inserez noua coordonata la inceputul cozii deoarece nu am 'cheltuit' nimic in plus,
            // vreau sa incerc sa continui pe traseul curent
